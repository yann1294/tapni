...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  Stream<ProfileState> mapEventToState(
    ...
    } else if (event is SocialLinkChanged) {
      yield* _mapSocialLinkChangedToState(event.socialLink);
  ...
  Stream<ProfileState> _mapSocialLinkChangedToState(String socialLink) async* {
    yield state.update(
      isSocialLink: socialLink == null,
    );
  }
  ...
  Stream<ProfileState> _mapSubmittedToState({
    ...
    try {
      await _userRepository.profileSetup(
          photo, userId, name, gender, interestedIn, socialLink, age, location);
